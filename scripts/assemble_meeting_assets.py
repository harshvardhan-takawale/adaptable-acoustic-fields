...
def main():
    ...
    # Chunk 3.8: if a presentation-resolution version (≥ 1500 px on long edge)
    # is already at the destination, leave it alone — `scripts/make_06_latent_manifold.py`
    # writes that version directly. Otherwise fall back to copying the source.
    dst_06 = out / "06_latent_manifold.png"
    src_06 = SWEEP_ROOT / "C1_film/latent_probe/figures/latent_pca_1d.png"
    presentation_06 = False
    if dst_06.exists():
        try:
            from PIL import Image
            w, h = Image.open(dst_06).size
            if max(w, h) >= 1500:
                presentation_06 = True
                asset_status["06_latent_manifold.png"] = "already-present (≥ 1500 px)"
        except Exception:
            pass
    if not presentation_06:
        if _maybe_copy(src_06, dst_06):
            asset_status["06_latent_manifold.png"] = "copied (source-resolution; rerun make_06_latent_manifold for presentation res)"
        else:
    ...
    _write_caption(out / "06_latent_manifold_caption.md",
        ...
        "unseen L, the spectrum rendering doesn't follow exactly.\n")

    # 05a — spatial mode shapes at L=5.25 (Chunk 3.8 headliner).
    # Generated by scripts/make_05a_spatial_modes.py; just confirm it's there.
    if (out / "05a_spatial_modes_L5_25.png").exists():
        asset_status["05a_spatial_modes_L5_25.png"] = "already-present"
    else:
        asset_status["05a_spatial_modes_L5_25.png"] = "missing (run scripts.make_05a_spatial_modes)"
    ...
    # 08 — progress trajectory (Chunk 3.8 closing optimism slide).
    if (out / "08_progress_trajectory.png").exists():
        asset_status["08_progress_trajectory.png"] = "already-present"
    else:
        asset_status["08_progress_trajectory.png"] = "missing (run scripts.make_08_trajectory)"

    # Top-level manifest.
    readme = ["# Meeting deck assets — Chunk 3.8", ""]
    readme.append("Each asset has a corresponding `*_caption.md` with an honest, ")
    readme.append("1-2 sentence description of what the plot shows. See ")
    readme.append("`DECK_NARRATIVE.md` for per-slide talking points + anticipated Q&A.")
    readme.append("")
    ...
    for k, v in asset_status.items():
        readme.append(f"| `{k}` | {v} |")
    readme.append("")
    readme.append("## Recommended deck order (10 slides)")
    readme.append("")
    readme.append("1. **Title slide** — \"Adaptable Acoustic Fields: Zero-Shot Modal Generalization in 2D Shoebox Rooms\"")
    readme.append("2. **Motivation** — adaptable spatial audio; per-scene retraining is expensive; Phase-1 scope")
    readme.append("3. **Method overview** — 2D port of INFER + auto-decoder + linear L-head + latent jitter")
    readme.append("4. **Foundation: single-room baseline** — `02_single_room_baseline.png` (modal MAE 0.34-0.58 Hz)")
    readme.append("5. **Multi-room training works** — `03_multi_room_training.png` (1.29-1.70 dB val LSD)")
    readme.append("6. **Latent learned geometry** — `06_latent_manifold.png` (C1 PC1-vs-L R² = 0.987)")
    readme.append("7. **Spatial mode shapes at unseen L** — two-panel: `05a_spatial_modes_L5_25.png` + `05_spatial_nodes_grid.png`")
    readme.append("8. **Modal peak tracking** — `04_zero_shot_modal_tracking.png` (1.04 Hz MAE on 31 pairs)")
    readme.append("9. **Data density is the lever** — `08_progress_trajectory.png` (3.7 → 2.55 dB across chunks)")
    readme.append("10. **Limitations + next steps** — text slide (22% modal recall; diffuse > 250 Hz still missing; 4 ranked next steps)")
    readme.append("")
    readme.append("Optional Slide 11: audio demo (one of the 3 morphing WAVs in `07_audio_demo/`).")
    readme.append("")
    readme.append("## Known limitations (call out honestly in the talk)")
    readme.append("- Full-band held LSD remains 4-5 dB on every config (modal-only is the success).")
    readme.append("- Modal-tracking recall is ~22%: the peaks we commit to are accurate (MAE 1.04 Hz), but we miss the majority of analytical modes.")
    readme.append("- Chunk 3.7's I1 modal LSD 2.55 dB is the project best; L=5.25 and L=5.75 are within 0.3 dB of the 2 dB target, but 0/6 unseen L are below it.")
