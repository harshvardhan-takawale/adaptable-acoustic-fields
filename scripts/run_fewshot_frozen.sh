...
#
#   bash scripts/run_fewshot_frozen.sh <RUN_ID> [SLURM_DEP]
# e.g. P3_45rooms_4gpu (density-45) | density_90 | density_150 | density_250
# Optional SLURM_DEP (e.g. afterok:7047710) is passed to each sbatch so the
# few-shot jobs pend until the density's training completes — lets the whole
# secondary column run unattended inside the autonomous chain.
set -euo pipefail
cd /fs/nexus-projects/multimodal_recon/adaptable-acoustic-fields
RUN_ID="${1:?usage: $0 <RUN_ID> [SLURM_DEP]}"
DEP="${2:-}"
TRAIN_OUT="outputs/multi_room_3d/${RUN_ID}"
# When a dependency is given, the train dir may not exist yet (jobs pend) — only
# require it for immediate (un-gated) submission.
if [ -z "${DEP}" ] && [ ! -d "${TRAIN_OUT}" ]; then
    echo "ERROR: ${TRAIN_OUT} does not exist (train not done?)"; exit 1
fi
...
for row in "${ROWS[@]}"; do
    ...
    if [ -n "${DEP}" ]; then
        sbatch --dependency="${DEP}" scripts/slurm/zero_shot_3d_eval.sh "${RUN_ID}" "${L}" "${W}" "${H}" >/dev/null
    else
