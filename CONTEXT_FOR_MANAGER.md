...
**Last updated**: Chunk 3.6 complete — 2026-05-11

## Project state

- **Phase**: 1 (2D shoebox sweep, 0–2 kHz, Track A — science).
- **Chunks completed**: Chunks 0/1/1.5/2/3 plus **Chunk 3.5/3.5+ (R0-R8 sweep)** plus **Chunk 3.6 (band-limited eval + 5 inner-loop variants + FiLM/latent-jitter retrains)**. All 9 R-runs + both C-runs complete.
...
- **Headline Chunk-3.6 result (negative, but definitive)**: across **11 configurations × up to 5 inner-loop strategies × 6 unseen L = 66+ (config, L) pairs, exactly 0 meet the 2 dB modal target**. Modal-regime LSD clusters tightly at **3.5-3.7 dB** across all configs (R0-R8, C1 FiLM, C2 latent jitter) and all inner-loop strategies (B1 baseline, B3 longer iters, B4 multi-restart, B5 nearest-train init, B6 simplex). Best: **C2_latent_jitter + B6 at 3.51 dB modal** — only 0.01 dB ahead of R6 + B6. FiLM and latent jitter improve in-distribution val LSD (1.38 / 1.43 dB — best of any config) but DON'T move zero-shot. The bottleneck is **the auto-decoder paradigm's reliance on a 7-room training set**: the latent-to-spectrum mapping is genuinely ambiguous at unseen L and neither inner-loop optimisation nor architectural smoothness breaks that ambiguity. Recommended pivot: denser training sweep (15+ rooms at ~0.2 m L spacing) and/or hyper-network conditioning. See `tasks/CHUNK_3_6_RESULTS.md` for the full analysis + 5 ranked next-iteration recommendations.
...
- **Read first (Chunk 3.6)**: [tasks/CHUNK_3_6_RESULTS.md](tasks/CHUNK_3_6_RESULTS.md) — the writeup with prose interpretation of all three tracks and the 5 ranked next-iteration recommendations. Then [outputs/multi_room/sweep/band_limited_summary.md](outputs/multi_room/sweep/band_limited_summary.md) (Track A modal/transition/diffuse table), [outputs/inner_loop_experiments/SUMMARY.md](outputs/inner_loop_experiments/SUMMARY.md) (Track B 5-variant table), [outputs/multi_room/sweep/SWEEP_SUMMARY.md](outputs/multi_room/sweep/SWEEP_SUMMARY.md) (11-row cross-run summary R0-R8 + C1 + C2).
