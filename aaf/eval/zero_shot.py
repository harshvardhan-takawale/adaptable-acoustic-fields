...
def zero_shot_adapt(
    ...
    if n_obs_receivers != 8:
        raise NotImplementedError(f"only n_obs_receivers=8 is supported; got {n_obs_receivers}")
    obs_idx = OBS_INDICES.copy()
    held_idx = np.array([i for i in range(receiver_pos.shape[0]) if i not in set(obs_idx)],
                        dtype=np.int64)
    ...
    # Initialise z_star fresh.
    torch.manual_seed(0)
    z_star = nn.Parameter(
        torch.randn(latent_dim, device=device) / math.sqrt(latent_dim)
    )

    # Tensors for the observed batch.
    rx_obs_t = torch.from_numpy(receiver_pos[obs_idx]).to(device)   # [8, 2]
    tx_obs_t = torch.from_numpy(np.tile(src, (rx_obs_t.size(0), 1))).to(device)
    H_obs_t = torch.from_numpy(H_target_all_np[obs_idx]).to(device)  # [8, n_freq]
    ...
    for step in range(n_adapt_iters):
        z_s = z_star.unsqueeze(0).expand(rx_obs_t.size(0), -1)
    ...
    # Eval mode for repeatable held-out forward.
    renderer.eval()
    with torch.no_grad():
        z_s_static = z_star.detach().unsqueeze(0)
        # Render all 64 receivers in chunks.
    ...
    # Save z_star + metrics + figures.
    torch.save(z_star.detach().cpu(), output_dir / "z_star.pt")
    metrics = {
        "L": L, "W": W,
    ...
    if loss_curve:
        ...
        ax1.set_title(f"L={L:.2f} m UNSEEN — z_star adaptation (final norm {z_star.detach().norm():.3f})")
        fig.tight_layout()
