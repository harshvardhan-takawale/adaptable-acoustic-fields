...
def _all_sweep_configs():
    return sorted(
        list(SWEEP_DIR.glob("R*.yaml"))
        + list(SWEEP_DIR.glob("C*.yaml"))
        + list(SWEEP_DIR.glob("D*.yaml"))
    )


def test_yamls_present():
    """R0-R5 (Chunk 3.5) + R6-R8 (Chunk 3.5+) + C1-C2 (Chunk 3.6) +
    D1-D2 (Chunk 3.7) = 13 sweep YAMLs."""
    ...
    expected |= {"D1_dense15", "D2_filmlora"}                            # Chunk-3.7
...
def test_model_from_yaml_matches_cfg(yaml_path):
    ...
    #   film / film_lora     → 2 * (2*n_levels)   (z_s injected via FiLM, not concat)
    sigma_feat = 2 * (2 * int(cfg["n_levels"]))
    expected_sigma_in = sigma_feat if conditioning_type in ("film", "film_lora") \
